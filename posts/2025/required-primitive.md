...
Etter en del graving kom jeg fram til at en ikke-nullbar "primitiv" verdi i Kotlin, vil kompileres til en `boolean ferdig` primitiv i byte-koden. Nullbar, `val ferdig: Boolean?`,  vil blir kompilert til en `Boolean` i byte-kode. Java-primitiver, i motsetning til objekt(referanser) har alltid en default-verdi, false for boolean hvis en verdi ikke er blitt tilordnet. Det viser seg at deserialiseringen derfor ikke feiler om propertyen mangler i Json-grafen fordi den likevel har fått en (default) verdi i objektet.
